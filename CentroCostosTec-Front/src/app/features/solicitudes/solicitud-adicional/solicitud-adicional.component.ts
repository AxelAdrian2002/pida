...
@Component({
  selector: 'app-solicitud-adicional',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule],
  template: `
  <div class="p-4">
    <h5 class="mb-4"><i class="fas fa-address-card me-2 text-primary"></i>Solicitud de Asignación Adicional</h5>
    <div class="card shadow-sm mb-3">
      <div class="card-body d-flex flex-wrap gap-2">
        <button class="btn btn-outline-primary" (click)="descargarPlantilla()" [disabled]="loading">Obtener plantilla</button>
        <label class="btn btn-outline-secondary mb-0">
          Cargar archivo
          <input type="file" accept=".xlsx" hidden (change)="onFileSelected($event)">
        </label>
        <button class="btn btn-primary" (click)="procesar()" [disabled]="loading || !archivoB64">Procesar</button>
      </div>
    </div>
    ...
    <div class="card border-danger mb-3">
      <div class="card-body py-3">
        ...
        <ul class="small mb-0 mt-2">
          <li>Para generar asignaciones adicionales, valida que el colaborador exista y esté activo.</li>
        </ul>
      </div>
    </div>
...
export class SolicitudAdicionalComponent {
  loading = false;
  exitoMsg = '';
  errorMsg = '';
  archivoNombre = '';
  ...
  descargarPlantilla(): void {
    ...
    this.exitoMsg = '';
    this.errorMsg = '';
    this.SolicitudService.getPlantilla('adicional').subscribe({
      next: (blob) => {
        this.descargarBlob(blob, 'plantilla_tarjeta_adicionales.xlsx');
  ...
  procesar(): void {
    ...
    this.loading = true;
    this.SolicitudService.procesaPedido('adicional', this.archivoB64).subscribe({
      next: res => {
        ...
        if (ok && this.pedidoProcesado) {
          ...
          this.router.navigate(['/solicitudes/resumen'], { queryParams: { tipo: 'adicional' } });
        } else {
