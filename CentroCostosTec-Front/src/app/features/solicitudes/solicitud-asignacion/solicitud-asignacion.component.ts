...
@Component({
  selector: 'app-solicitud-asignacion',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule],
  template: `
  <div class="p-4">
    <h5 class="mb-4"><i class="fas fa-credit-card me-2 text-info"></i>Solicitud de Nueva Asignación</h5>
    <div class="card shadow-sm mb-3">
      <div class="card-body d-flex flex-wrap gap-2">
        <button class="btn btn-outline-info" (click)="descargarPlantilla()" [disabled]="loading">Obtener plantilla</button>
        <label class="btn btn-outline-secondary mb-0">
          Cargar archivo
          <input type="file" accept=".xlsx" hidden (change)="onFileSelected($event)">
        </label>
        <button class="btn btn-info text-white" (click)="procesar()" [disabled]="loading || !archivoB64">Procesar</button>
      </div>
    </div>
    ...
    <div class="card border-danger mb-3">
      <div class="card-body py-3">
        <div class="text-danger fw-semibold small">Favor de considerar los siguientes puntos para la correcta emisión de la credencial interna.</div>
        <ul class="small mb-0 mt-2">
          <li>La impresión de la tarjeta solo incluye 26 caracteres contando espacios.</li>
          <li>El nombre del empleado no debe contener acentos ni símbolos especiales.</li>
          <li>La letra Ñ debe sustituirse por N.</li>
        </ul>
      </div>
    </div>
    <div class="card shadow-sm" *ngIf="pedidoProcesado">
      ...
      <div class="card-body">
        <strong>Monto:</strong> {{ pedidoProcesado.montoTotal || 0 | currency:'MXN' }}
      </div>
    </div>
  </div>
  `
})
export class SolicitudAsignacionComponent {
  loading = false;
  exitoMsg = '';
  errorMsg = '';
  archivoNombre = '';
  ...
  descargarPlantilla(): void {
    ...
    this.exitoMsg = '';
    this.errorMsg = '';
    this.SolicitudService.getPlantilla('tarjeta').subscribe({
      next: (blob) => {
        this.descargarBlob(blob, 'plantilla_tarjeta_titulares.xlsx');
        this.exitoMsg = 'Plantilla solicitada correctamente.';
        this.loading = false;
      },
      error: err => {
        this.errorMsg = err.error?.mensaje || 'No fue posible obtener plantilla.';
        this.loading = false;
      }
    });
  }
  ...
  procesar(): void {
    ...
    this.loading = true;
    this.SolicitudService.procesaPedido('tarjeta', this.archivoB64).subscribe({
      next: res => {
        ...
        if (ok && this.pedidoProcesado) {
          ...
          this.router.navigate(['/solicitudes/resumen'], { queryParams: { tipo: 'tarjeta' } });
        } else {
          this.exitoMsg = 'Archivo procesado.';
        }
        this.loading = false;
      },
      error: err => {
